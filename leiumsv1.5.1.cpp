#include <iostream.h>
#include <conio.h>
#include <stdio.h>
#include <string.h>
#include <fstream.h>
#include <dos.h>
#include <stdlib.h>
#include <ctype.h>
#define equipments_file_path "EQ-DATA1.txt"
#define max_equipments 500

struct Equipment {
    char name[50];
    int quantity;
    char acquisitionDate[15];
    char condition[20];
    float usageHours;
    float depreciation;
    float availabilityRate;
} equipments[max_equipments];
int eqCounter = 0;

// ############################## HELPERS DIVIDERS & SPACES ETC. ##############################
void printLine(int length, int color, char symbol){ // LINE DIVIDER
    textcolor(color);
    for(int i = 0; i < length; i++){
        cprintf("%c",symbol);
    }
    textcolor(WHITE);
}
void error(char* msg){ textcolor(RED); cprintf("%s", msg); textcolor(WHITE); } // ERROR MESSAGE
void info(char* msg){ textcolor(YELLOW); cprintf("%s", msg); textcolor(WHITE); } // INFO MESSAGE
void success(char* msg){ textcolor(LIGHTGREEN); cprintf("%s", msg); textcolor(WHITE); } // SUCCESS MESSAGE
void screenTemplate(int color, int x, int y, char* title){ //SCREEN TEMPLATE
    gotoxy(1,1); printLine(80, 11, '=');gotoxy(1,3); printLine(80, 11, '-'); gotoxy(1,24); printLine(80, 11, '=');
    textcolor(color); gotoxy(x,y); highvideo(); cprintf(title); normvideo(); gotoxy(1,4); textcolor(WHITE);
}
void returnMenu(){ // RETURN MENU
    gotoxy(43,23); textcolor(YELLOW); cprintf("Press any key to return to main MENU."); textcolor(WHITE);
    getch(); return;
}
void clearLines(int start, int end) { // CLEAR MULTIPLE LINES
    for (int y = start; y <= end; y++) {
        gotoxy(1, y);
        clreol();
    }
}
int valid (char* type, char* value) { // Check input type (int n float)
    int i, dotCount = 0, start = 0;
    if (value[0] == '-') start = 1;
    if (strcmp(type, "int") == 0) { // check for int
        for (i = start; i < strlen(value); i++) {
            if (!isdigit((unsigned char)value[i])) return 0;
        }
        return 1;
    }
    else if (strcmp(type, "float") == 0) { // check for float
        for (i = start; i < strlen(value); i++) {
            if (value[i] == '.') {
                dotCount++;
                if (dotCount > 1) return 0; // more than one dot is invalid
            } 
            else if (!isdigit((unsigned char)value[i])) return 0; // anything not digit or '.' is invalid
        }
        if (strlen(value) == start + 1 && value[start] == '.') return 0; // Make sure not just '.' or '-.'
        return 1;
    }
    return 0; // if no matched type
}
int isCancel (char* input) {
    char lower[1000];
    for (int i = 0; input[i]; i++) lower[i] = tolower(input[i]); lower[strlen(input)] = '\0';
    return strcmp(lower, "icancel") == 0;
}

// ############################## DISPLAY MENU ##############################
void displayMenu(){
    // SCREEN DISPLAY
    screenTemplate(9, 35, 2, "MAIN MENU");
    cout << "[1] Display Equipments\n";
    cout << "[2] Search / View / Delete Equipments\n";
    cout << "[3] Add Equipments\n";
    cout << "[0] Exit\n";
}
// ############################## SAVE EQUIPMENTS ##############################
void saveEquipment(Equipment e) {
    ofstream file(equipments_file_path, ios::app);

    if (!file) {
        gotoxy(1,23); textcolor(RED); cprintf("Error opening file!\n"); textcolor(WHITE);
        return;
    }

    file << e.name << "|"
         << e.quantity << "|"
         << e.acquisitionDate << "|"
         << e.condition << "|"
         << e.usageHours << "|"
         << e.depreciation << "|"
         << e.availabilityRate << "\n";

    file.close();
    textcolor(2); gotoxy(1,23); clreol(); cprintf("Equipment saved successfully!"); textcolor(15);
}
void saveAllEquipment() {
    ofstream file(equipments_file_path);
    if (!file) {
        gotoxy(1,23); textcolor(RED); cprintf("Error opening file!\n"); textcolor(WHITE);
        return;
    }
    for (int i = 0; i < eqCounter; i++) {
        file << equipments[i].name << "|"
             << equipments[i].quantity << "|"
             << equipments[i].acquisitionDate << "|"
             << equipments[i].condition << "|"
             << equipments[i].usageHours << "|"
             << equipments[i].depreciation << "|"
             << equipments[i].availabilityRate << "\n";
    }
    file.close();
}
// ############################## LOAD EQUIPMENTS ##############################
void loadEquipment() {
    ifstream file(equipments_file_path);
    if (!file) return;
    char line[256];
    eqCounter = 0;

    while (!file.eof()) {
        file.getline(line, 255, '\n'); // get data from file line by line
        if (strlen(line) == 0) continue; // to skip empty lines
        if (eqCounter >= max_equipments) break; // to avoid overflowing data

        char *token = strtok(line, "|"); if (!token) continue;
        strcpy(equipments[eqCounter].name, token); // get name

        token = strtok(NULL, "|"); if (!token) continue;
        equipments[eqCounter].quantity = atoi(token); // get quantity

        token = strtok(NULL, "|"); if (!token) continue;
        strcpy(equipments[eqCounter].acquisitionDate, token); // get date acquirred

        token = strtok(NULL, "|"); if (!token) continue;
        strcpy(equipments[eqCounter].condition, token); // get condition

        token = strtok(NULL, "|"); if (!token) continue;
        equipments[eqCounter].usageHours = atof(token); // get usage hours

        token = strtok(NULL, "|"); if (!token) continue;
        equipments[eqCounter].depreciation = atof(token); // get depriciation

        token = strtok(NULL, "|"); if (!token) continue;
        equipments[eqCounter].availabilityRate = atof(token); // get availability rate
        eqCounter++; // increment counter per line
    }
    file.close(); // close file after reading data
}
// ############################## COUNT FILE LINES (DATA COUNTER) ##############################
int countLines(const char *filename) {
    ifstream file(filename);
    if (!file) return 0;

    int count = 0;
    char line[256];
    while(file.getline(line, sizeof(line))) count++;
    file.close();
    return count;
}
// ################################################## DISPLAY EQUIPMENTS ##################################################
void displayEquipments() {
    struct date d; getdate(&d); // get date
    struct time t; gettime(&t); // get time
    clrscr(); screenTemplate(9, 3, 2, "> EQUIPMENTS RECORD"); // screen template and title
    textcolor(WHITE); gotoxy(65,2); printf("DATE: %02d-%02d-%04d", d.da_day, d.da_mon, d.da_year); // display date
    gotoxy(1,4); textbackground(LIGHTGRAY); textcolor(BLACK); // text format for table header
    cprintf(" %-23s %-5s %-11s %-12s %7s %7s %7s ", "NAME", "QTY", "DATE", "CONDITION", "HOURS", "DEP%", "AVAIL%"); // table header
    textbackground(BLACK); textcolor(WHITE); normvideo(); // reset text formatting
    loadEquipment(); // load data

    if (eqCounter <= 0) { // check for empty data
        gotoxy(1,5); info("No records found.");
        gotoxy(1,23); error("File is empty or not found.");
        returnMenu(); return;
    }

    int perPage = 15; // number of data per page
    int page = 0; // page number
    int totalPages = (eqCounter + perPage - 1) / perPage; // total pages

    while (1) {
        clrscr();
        screenTemplate(9,3,2, "> EQUIPMENTS RECORD");
        textcolor(WHITE); gotoxy(65,2); printf("DATE: %02d-%02d-%04d", d.da_day, d.da_mon, d.da_year);
        gotoxy(1,4); textbackground(LIGHTGRAY); textcolor(BLACK);
        cprintf(" %-23s %-5s %-11s %-12s %7s %7s %7s ", "NAME", "QTY", "DATE", "CONDITION", "HOURS", "DEP%", "AVAIL%"); // table header
        textbackground(BLACK); textcolor(WHITE); normvideo();

        int start = page * perPage;
        int end = start + perPage;
        if (end > eqCounter) end = eqCounter;
        int y = 5;

        for (int i = start; i < end; i++) {
            gotoxy(1, y++); lowvideo();
            cprintf(" %-23s %-5d %-11s %-12s %7.2f %7.2f %7.2f ",
                    equipments[i].name,
                    equipments[i].quantity,
                    equipments[i].acquisitionDate,
                    equipments[i].condition,
                    equipments[i].usageHours,
                    equipments[i].depreciation,
                    equipments[i].availabilityRate);
            normvideo();
        }

        gotoxy(1,20); printLine(80,11,'-');
        gotoxy(1,21); textcolor(YELLOW);
        cprintf("%02d-%02d out of %d data.  Page %d/%d", 
                start + 1, end, eqCounter, page + 1, totalPages);
        textcolor(CYAN);
        cprintf("  [LEFT/RIGHT arrows for pages] [ESC exit]");
        textcolor(WHITE);

        // Key Controls
        int ch = getch();
        if (ch == 27) break; // ESC key to exit display

        if (ch == 0 || ch == 224) { // extended key
            ch = getch(); // read actual key
            if (ch == 77 && page < totalPages - 1) { // Right arrow for next page
                page++;
            } 
            else if (ch == 75 && page > 0) { // Left arrow for previous page
                page--;
            }
        }
    }
}

// ################################################## SEARCH EQUIPMENTS ##################################################
void searchEquipmentByName() { // ########## SEARCH BY NAME ##########
    struct date d; getdate(&d);
    struct time t; gettime(&t);
    clrscr();
    screenTemplate(9, 3, 2, "> SEARCH EQUIPMENT BY NAME");
    gotoxy(35, 2); info("Enter equipment name or [icancel] to exit.");

    char ans1[50], lowerAns1[50];
    int found = 0;

get_ans1:
    gotoxy(1,4); clreol(); cout << "Enter name to search: "; gets(ans1);
    for (int x = 0; ans1[x]; x++) lowerAns1[x] = tolower(ans1[x]);
    lowerAns1[strlen(ans1)] = '\0';

    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') {
        gotoxy(36,4); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_ans1;
    }
    else if (isCancel(ans1)) {
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }

    loadEquipment();
    if (eqCounter <= 0) {
        gotoxy(1,5); info("No records found.");
        gotoxy(1,23); error("File is empty or not found.");
        returnMenu(); return;
    }

    Equipment matches[200];
    int matchCount = 0;
    for (int z = 0; z < eqCounter; z++) {
        char lowerName[50];
        for (int j = 0; equipments[z].name[j]; j++) lowerName[j] = tolower(equipments[z].name[j]);
        lowerName[strlen(equipments[z].name)] = '\0';
        if (strstr(lowerName, lowerAns1)) matches[matchCount++] = equipments[z];
    }

    if (matchCount == 0) {
        gotoxy(1,23); error("No matches found.");
        returnMenu(); return;
    }

    int perPage = 15, page = 0, totalPages = (matchCount + perPage - 1) / perPage;
    int selected = 0; // highlighted record index (within current page)

    while (1) {
        clrscr();
        screenTemplate(9,3,2, "> SEARCH RESULTS");
        textcolor(WHITE); gotoxy(65,2); printf("DATE: %02d-%02d-%04d", d.da_day, d.da_mon, d.da_year);
        gotoxy(1,4); textbackground(LIGHTGRAY); textcolor(BLACK);
        cprintf(" %-24s %-5s %-11s %-12s %7s %7s %7s", "NAME", "QTY", "DATE", "CONDITION", "HOURS", "DEP%", "AVAIL%");
        textbackground(BLACK); textcolor(WHITE); normvideo();

        int start = page * perPage;
        int end = start + perPage;
        if (end > matchCount) end = matchCount;
        int y = 5;

        for (int i = start; i < end; i++) {
            if (i == start + selected) {
                textbackground(BLUE); textcolor(WHITE); // highlight row
            } else {
                textbackground(BLACK); textcolor(WHITE);
            }
            gotoxy(1, y++);
            cprintf(" %-24s %-5d %-11s %-12s %7.2f %7.2f %7.2f",
                    matches[i].name,
                    matches[i].quantity,
                    matches[i].acquisitionDate,
                    matches[i].condition,
                    matches[i].usageHours,
                    matches[i].depreciation,
                    matches[i].availabilityRate);
        }

        textbackground(BLACK); textcolor(WHITE);
        gotoxy(1,20); printLine(80,11,'-');
        gotoxy(1,21); textcolor(YELLOW);
        cprintf("%02d-%02d out of %d results. Page %d/%d", start + 1, end, matchCount, page + 1, totalPages);
        gotoxy(1,22); textcolor(LIGHTCYAN);
        cprintf("[UP/DOWN select] [LEFT/RIGHT page] [DEL delete] [ESC exit]");
        textcolor(WHITE);

        int ch = getch();

        if (ch == 27) break; // ESC → exit

        if (ch == 0 || ch == 224) {
            ch = getch();
            if (ch == 80 && selected < (end - start - 1)) selected++;  // down
            else if (ch == 72 && selected > 0) selected--;             // up
            else if (ch == 77 && page < totalPages - 1) {              // next page
                page++; selected = 0;
            }
            else if (ch == 75 && page > 0) {                           // prev page
                page--; selected = 0;
            }
            else if (ch == 83) { // Delete key
                int delIndex = start + selected;
                gotoxy(1,23); clreol();
                printf("Delete '%s'? (Y/N): ", matches[delIndex].name);
                char confirm = getch();
                if (tolower(confirm) != 'y') continue;

                // Delete from main equipments[]
                for (int i = 0; i < eqCounter; i++) {
                    if (strcmpi(equipments[i].name, matches[delIndex].name) == 0) {
                        for (int j = i; j < eqCounter - 1; j++)
                            equipments[j] = equipments[j + 1];
                        eqCounter--;
                        break;
                    }
                }
                saveAllEquipment();
                gotoxy(1,23); success("Record deleted successfully."); delay(600);

                // Refresh search results
                loadEquipment();
                matchCount = 0;
                for (int z = 0; z < eqCounter; z++) {
                    char lowerName[50];
                    for (int j = 0; equipments[z].name[j]; j++) lowerName[j] = tolower(equipments[z].name[j]);
                    lowerName[strlen(equipments[z].name)] = '\0';
                    if (strstr(lowerName, lowerAns1)) matches[matchCount++] = equipments[z];
                }
                if (matchCount == 0) {
                    gotoxy(1,23); error("No remaining matches found. Exiting search.");
                    getch(); break;
                }
                totalPages = (matchCount + perPage - 1) / perPage;
                if (page >= totalPages) page = totalPages - 1;
                if (selected >= (end - start)) selected = 0;
            }
        }
    }
}

// ################################################## ADD EQUIPMENTS ##################################################
void addEquipments() {
    clrscr();
    char ans1[1000], lAns1[1000], ans2[1000];
    int i;
    Equipment eq;
get_data: // INPUT DATA
    screenTemplate(9, 3, 2, "> ADD EQUIPMENTS"); gotoxy(35, 2); info("Enter necessary data or [icancel] to exit.");

get_name: // INPUT NAME
    gotoxy(1,4); clreol(); cout << "Equipment name \033[1;35m[20 characters max]\033[0m: "; gets(ans1);

    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') { // empty check
        gotoxy(36,4); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_name;
    }
    else if (isCancel(ans1)) { // cancel check
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if (strlen(ans1) > 20) { // length check
        gotoxy(36,4); error(" Too long."); info(" Press any key to continue."); getch(); goto get_name;
    } else strcpy(eq.name, ans1); // store to structure

get_qty: // INPUT QUANTITY
    gotoxy(1,5); clreol(); cout << "Quantity       \033[1;35m[    1 - 9999     ]\033[0m: "; gets(ans1);

    if (strlen(ans1) == 0) { // empty check
        gotoxy(36,5); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_qty;
    }
    else if (isCancel(ans1)) { // cancel check
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if (valid("int", ans1) == 0 || ans1[0] == '-') { // input type check
        gotoxy(36,5); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_qty;
    } else {
        eq.quantity = atoi(ans1); // store to structure
        if (eq.quantity > 9999 || eq.quantity < 1) { // value check
            gotoxy(36,5); error(" Out of range."); info(" Press any key to continue."); getch(); goto get_qty;
        }
    }

get_date: // INPUT ACQUISITION DATE
    gotoxy(1,6); clreol(); cout << "Date acquired  \033[1;35m[   DD/MM/YYYY    ]\033[0m: "; gets(ans1);

    if (strlen(ans1) == 0) { // check if empty
        gotoxy(36,6); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_date;
    }
    else if (isCancel(ans1)) { // check for cancel
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if (strlen(ans1) != 10) { // length check
        gotoxy(36,6); error(" Invalid format."); info(" Press any key to continue."); getch(); goto get_date;
    } strcpy(eq.acquisitionDate, ans1); // Store to structure

get_condition: // INPUT CONDITION
    gotoxy(1,7); clreol(); cout << "Condition   \033[1;35m[very bad < very good]\033[0m: "; gets(ans1);
    for (i = 0; ans1[i]; i++) { lAns1[i] = tolower(ans1[i]); } lAns1[i] = '\0';
    int validCondition =
        strcmp(lAns1, "very bad") == 0 ||
        strcmp(lAns1, "bad") == 0 ||
        strcmp(lAns1, "good") == 0 ||
        strcmp(lAns1, "very good") == 0;
    if (strlen(ans1) == 0) { // check if empty
        gotoxy(36,7); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_condition;
    }
    else if (isCancel(ans1)) { // check for cancel
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if ( !validCondition ) { // input check
        gotoxy(36,7); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_condition;
    } else { strcpy(eq.condition, lAns1); } // Store to condition

get_hoursUsed: // INPUT HOURS USED
    gotoxy(1,8); clreol(); cout << "Hours used     \033[1;35m[    0 - 9999     ]\033[0m: "; gets(ans1);

    if (strlen(ans1) == 0) { // empty check
        gotoxy(36,8); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_hoursUsed;
    }
    else if (isCancel(ans1)) { // cancel check
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if (valid("int", ans1) == 0 && valid("float", ans1) == 0) { // input type check
        gotoxy(36,8); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_hoursUsed;
    } else {
        eq.usageHours = atof(ans1); // store to structure
        if (eq.usageHours < 0 || eq.usageHours > 9999) { // value check
            gotoxy(36,8); error(" Out of range."); info(" Press any key to continue."); getch(); goto get_hoursUsed;
        }
    }

get_dep: // INPUT DEPRICIATION RATE
    gotoxy(1,9); clreol(); cout << "Depreciation Rate(%) \033[1;35m[  0 - 100  ]\033[0m: "; gets(ans1);

    if (strlen(ans1) == 0) { // empty check
        gotoxy(36,9); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_dep;
    }
    else if (isCancel(ans1)) { // cancel check
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if (valid("int", ans1) == 0 && valid("float", ans1) == 0) { // input type check
        gotoxy(36,9); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_dep;
    } else {
        eq.depreciation = atof(ans1); // store to structure
        if (eq.depreciation < 0 || eq.depreciation > 100) { // value check
            gotoxy(36,9); error(" Out of range."); info(" Press any key to continue."); getch(); goto get_dep;
        }
    }

get_avail: // INPUT AVAILABILITY RATE
    gotoxy(1,10); clreol(); cout << "Availability Rate(%) \033[1;35m[  0 - 100  ]\033[0m: "; gets(ans1);

    if (strlen(ans1) == 0) { // empty check
        gotoxy(36,10); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_avail;
    }
    else if (isCancel(ans1)) { // cancel check
        gotoxy(1,23); cout << "Operation cancelled.";
        returnMenu(); return;
    }
    else if (valid("int", ans1) == 0 && valid("float", ans1) == 0) { // input type check
        gotoxy(36,10); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_avail;
    } else {
        eq.availabilityRate = atof(ans1); // store to structure
        if (eq.availabilityRate < 0 || eq.availabilityRate > 100) { // value check
            gotoxy(36,10); error(" Out of range."); info(" Press any key to continue."); getch(); goto get_avail;
        }
    }

    gotoxy(1,11); printLine(80,11,'-'); // divider line
    gotoxy(1,12); cout << "[1] Confirm and Save";
    gotoxy(1,13); cout << "[2] Retry";
    gotoxy(1,14); cout << "[3] Exit (not saved)";
get_ans2:
    gotoxy(1,22); clreol(); cout << "Enter your choice [1-3]: "; gets(ans2);
    if(strcmp(ans2, "1") == 0){
        saveEquipment(eq);
    } else if(strcmp(ans2, "2") == 0){
        clearLines(4,23);
        goto get_data;
    } else if(strcmp(ans2, "3") == 0){
        return;
    } else {
        gotoxy(1,23); error("Invalid Input."); goto get_ans2;
    }

    returnMenu(); return;
}
// ################################################## REMOVE EQUIPMENTS ##################################################
// void removeEquipments() {
//     clrscr();
//     struct date d; getdate(&d);
//     struct time t; gettime(&t);
//     screenTemplate(9, 3, 2, "> REMOVE EQUIPMENT RECORD");
//     loadEquipment();

//     if (eqCounter <= 0) {
//         gotoxy(1,5); info("No records found.");
//         gotoxy(1,23); error("File is empty or not found.");
//         returnMenu(); return;
//     }

//     // --- Display list of equipments with record numbers ---
//     gotoxy(1,4); textbackground(LIGHTGRAY); textcolor(BLACK);
//     cprintf(" %-3s %-20s %-5s %-11s %-12s %7s %7s %7s",
//             "NO", "NAME", "QTY", "DATE", "CONDITION", "HOURS", "DEP%", "AVAIL%");
//     textbackground(BLACK); textcolor(WHITE); normvideo();

//     int y = 5;
//     for (int i = 0; i < eqCounter && y < 20; i++) {
//         gotoxy(1, y++); lowvideo();
//         cprintf(" %-3d %-20s %-5d %-11s %-12s %7.2f %7.2f %7.2f",
//                 i + 1,
//                 equipments[i].name,
//                 equipments[i].quantity,
//                 equipments[i].acquisitionDate,
//                 equipments[i].condition,
//                 equipments[i].usageHours,
//                 equipments[i].depreciation,
//                 equipments[i].availabilityRate);
//         normvideo();
//     }

//     gotoxy(1,20); printLine(80,11,'-');
//     gotoxy(1,21); textcolor(YELLOW);
//     cprintf("Enter record number to remove [1-%d] or 0 to cancel:", eqCounter);
//     textcolor(WHITE);

//     char ans[10];
//     int index;
// get_choice:
//     gotoxy(1,22); clreol(); cprintf("Choice: ");
//     gets(ans);

//     if (strlen(ans) == 0) { gotoxy(10,22); error("Empty input."); goto get_choice; }
//     if (!valid("int", ans)) { gotoxy(10,22); error("Invalid input."); goto get_choice; }

//     index = atoi(ans);
//     if (index == 0) {
//         gotoxy(1,23); info("Operation cancelled.");
//         returnMenu(); return;
//     }
//     if (index < 1 || index > eqCounter) {
//         gotoxy(1,23); error("Out of range."); goto get_choice;
//     }

//     // Confirm deletion
//     gotoxy(1,23); clreol();
//     printf("Are you sure you want to delete '%s'? (Y/N): ", equipments[index - 1].name);
//     char confirm = getch();
//     if (tolower(confirm) != 'y') {
//         gotoxy(1,24); info("Cancelled.");
//         returnMenu(); return;
//     }

//     // --- Shift records up ---
//     for (int j = index - 1; j < eqCounter - 1; j++) {
//         equipments[j] = equipments[j + 1];
//     }
//     eqCounter--;

//     // --- Rewrite file with updated data ---
//     saveAllEquipment();

//     gotoxy(1,24); textcolor(GREEN); cprintf("Record removed successfully."); textcolor(WHITE);
//     returnMenu(); return;
// }
// // ################################################## TEMPLATE DONT DELETE ##################################################
// void displayEquipments(){
//     clrscr();
//     textcolor(WHITE);
//     cout << "Equipments Display.";
// }

int main(){
    char ans[1000];
    int option;
    do{
        clrscr();
        displayMenu();
    get_ans:
        gotoxy(1,22); clreol(); cout <<"Enter your choice [0-7]: "; gets(ans);
        if(strcmp(ans, "1") == 0) option = 1;
        else if(strcmp(ans, "2") == 0) option = 2;
        else if(strcmp(ans, "3") == 0) option = 3;
        // else if(strcmp(ans, "4") == 0) option = 4;
        // else if(strcmp(ans, "5") == 0) option = 5;
        // else if(strcmp(ans, "6") == 0) option = 6;
        // else if(strcmp(ans, "7") == 0) option = 7;
        else if(strcmp(ans, "0") == 0) option = 0;
        else {
            gotoxy(1,23);
            error("Invalid input.");
            goto get_ans;
        }
        switch (option){
        case 0:
            return 0;
        case 1:
            displayEquipments();
            break;
        case 2:
            searchEquipmentByName();
            break;
        case 3:
            addEquipments();
            break;
        // case 4:
        //     removeEquipments();
        //     break;
        default:
            break;
        }
    } while (option != 0);

    getch();
    return 0;
}
