...
void displayEquipments() {
    ...
    cprintf(" %-24s %-5s %-11s %-12s %7s %7s %7s", "NAME", "QTY", "DATE", "CONDITION", "HOURS", "DEP%", "AVAIL%"); // table header
    ...
    while (1) {
        ...
        cprintf(" %-24s %-5s %-11s %-12s %7s %7s %7s", 
                "NAME", "QTY", "DATE", "CONDITION", "HOURS", "DEP%", "AVAIL%");
...
void searchEquipmentByName() { // ########## SEARCH BY NAME ##########
    ...
    clrscr();
    screenTemplate(9, 3, 2, "> SEARCH EQUIPMENT BY NAME"); gotoxy(35, 2); info("Enter equipment name or [icancel] to exit.");

    char ans1[50], lowerAns1[50];
    int found = 0, y = 5;
get_ans1:
    gotoxy(1,4); clreol(); cout << "Enter name to search: "; gets(ans1); // get input for validaton
    for (int x = 0; ans1[x]; x++) lowerAns1[x] = tolower(ans1[x]);  // lowercase for searching
    ...
    gotoxy(1,4); textbackground(LIGHTGRAY); textcolor(BLACK); clreol();
    ...
    for (int i = 0; i < eqCounter; i++) {
        char lowerName[50];
        for (int j = 0; equipments[i].name[j]; j++) lowerName[j] = tolower(equipments[i].name[j]);
        lowerName[strlen(equipments[i].name)] = '\0';

        if (strstr(lowerName, lowerAns1)) {
            gotoxy(1,y++); lowvideo();
            cprintf(" %-24s %-5d %-11s %-12s %-7.2f %-7.2f %-7.2f",
                    equipments[i].name,
                    equipments[i].quantity,
            ...
            if (y >= 20) {
                gotoxy(1,20); printLine(80, 11, '-');
                gotoxy(1,21); textcolor(YELLOW); cprintf("%d out of %d data. (press any key to continue)", y - 5, eqCounter); textcolor(WHITE); getch(); clrscr();
    ...
    normvideo();
    if (found == 0) {
        gotoxy(1,23); clreol(); error("No matched found.");
    } else {
        gotoxy(1,23); clreol(); printf("%d equipments found.", found);
    }
    gotoxy(1,20); printLine(80,11,'-');
    gotoxy(1,21); textcolor(YELLOW); cprintf("%d out of %d data.", y - 5, eqCounter); textcolor(WHITE);
    returnMenu();
}
// ################################################## ADD EQUIPMENTS ##################################################
void addEquipments() {
...
// ################################################## REMOVE EQUIPMENTS ##################################################
void removeEquipments(){ // work in progress
    clrscr();
    textcolor(WHITE);
