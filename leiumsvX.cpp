...
struct Equipment {
    ...
    int inUseFlag; // 1 if in use, 0 if not
    char lastUsedStart[20]; // "DD/MM/YYYY  HH:MM"
...
// void cleanString(char *str) { // CLEAN STRINGS
//     int len = strlen(str);
...
void loadEquipment() {
    ...
    while (!file.eof()) {
        ...
        if (eqCounter >= max_equipments) break;

        char *token = strtok(line, "|");
        if (!token) continue;
        strcpy(equipments[eqCounter].name, token);

        token = strtok(NULL, "|");
        if (!token) continue;
        strcpy(equipments[eqCounter].acquisitionDate, token);

        token = strtok(NULL, "|");
        if (!token) continue;
        strcpy(equipments[eqCounter].condition, token);

        token = strtok(NULL, "|");
        if (!token) continue;
        equipments[eqCounter].usageHours = atof(token);

        token = strtok(NULL, "|");
        if (!token) continue;
        equipments[eqCounter].efficiency = atof(token);

        token = strtok(NULL, "|");
        if (!token) continue;
        equipments[eqCounter].availabilityRate = atof(token);

        token = strtok(NULL, "|");
        if (!token) continue;
        strcpy(equipments[eqCounter].status, token);

        token = strtok(NULL, "|");
        if (!token) continue;
        strcpy(equipments[eqCounter].category, token);

        // ===== NEW FIELDS (optional in old saves) =====
        token = strtok(NULL, "|");
        if (token) {
            equipments[eqCounter].inUseFlag = atoi(token);
        } else {
            equipments[eqCounter].inUseFlag = 0; // default
        }

        token = strtok(NULL, "|");
        if (token) {
            strcpy(equipments[eqCounter].lastUsedStart, token);
        } else {
            strcpy(equipments[eqCounter].lastUsedStart, "-");
        }
...
void viewEquipmentDetails(Equipment eq) {
    clrscr();
    int boxLeft = 15, boxTop = 3, boxWidth = 55, boxHeight = 16, i; // +2 height for extra lines
    ...
    // ===== NEW v1.7 INFO =====
    if (eq.inUseFlag == 1 && strcmp(eq.status, "in use") == 0) {
        textcolor(LIGHTRED);
        gotoxy(x, y++); printf("Currently In Use: YES");
        textcolor(LIGHTCYAN);
        gotoxy(x, y++); printf("Last Used Start : %s", eq.lastUsedStart);
    } else {
        textcolor(LIGHTGREEN);
        gotoxy(x, y++); printf("Currently In Use: NO");
        textcolor(LIGHTCYAN);
        gotoxy(x, y++); printf("Last Used Start : %s", strcmp(eq.lastUsedStart, "-") == 0 ? "—" : eq.lastUsedStart);
    }
    textcolor(WHITE);
    ...
    while (1) {
        ...
        if (ch == 0 || ch == 224) {   // extended key
            ...
            if (ch == 80 && selected < (end - start - 1)) selected++;               // DOWN
                ...
                // for (i = 0; i < eqCounter; i++) {
                //     // Defensive trim for CR/LF
                //     char cleanEquipName[50], cleanMatchName[50];
                //     strcpy(cleanEquipName, equipments[i].name);
                //     strcpy(cleanMatchName, matches[delIndex].name);
                //     cleanString(cleanEquipName); // small helper to remove trailing \r/\n/spaces
                //     cleanString(cleanMatchName);

                //     if (strcmpi(cleanEquipName, cleanMatchName) == 0) {
...
get_name: // ========== NAME ==========
    gotoxy(1,5); clreol(); info("[!] 20 characters max");
    gotoxy(1,4); clreol(); cout << "Equipment name  : "; gets(ans1);
    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') {
        gotoxy(19,4); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_name;
    } else if (isCancel(ans1)) {
        gotoxy(1,23); cout << "Operation cancelled."; returnMenu(); return;
    } else if (strlen(ans1) > 20) {
        gotoxy(19,4); error(" Too long."); info(" Press any key to continue."); getch(); goto get_name;
    } else strcpy(eq.name, ans1);

get_date: // ========== DATE ACQUIRED ==========
    gotoxy(1,6); clreol(); info("[!] Format (DD/MM/YYYY)");
    gotoxy(1,5); clreol(); cout << "Date acquired   : "; gets(ans1);
    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') {
        gotoxy(19,5); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_date;
    } else if (isCancel(ans1)) {
        gotoxy(1,23); cout << "Operation cancelled."; returnMenu(); return;
    } else if (!validDateFormat(ans1)) {
        gotoxy(19,5); error(" Invalid format."); info(" Press any key to continue."); getch(); goto get_date;
    } else strcpy(eq.acquisitionDate, ans1);

get_condition: // ========== CONDITION ==========
    gotoxy(1,7); clreol(); info("[!] Enter (very bad, bad, good, very good)");
    gotoxy(1,6); clreol(); cout << "Condition       : "; gets(ans1);
    for (i = 0; ans1[i]; i++) lowerA[i] = tolower(ans1[i]); lowerA[i] = '\0';
    int validCondition =
        strcmp(lowerA, "very bad") == 0 ||
        strcmp(lowerA, "bad") == 0 ||
    ...
    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') {
        ...
        gotoxy(19,6); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_condition;
    }
    strcpy(eq.condition, lowerA);

get_hoursUsed: // ========== HOURS USED ==========
    gotoxy(1,8); clreol(); info("[!] Range (0 - 9999)");
    gotoxy(1,7); clreol(); cout << "Hours used      : "; gets(ans1);
    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') {
        gotoxy(19,7); error(" Cannot be empty."); info(" Press any key to continue."); getch(); goto get_hoursUsed;
    } else if (isCancel(ans1)) {
        gotoxy(1,23); cout << "Operation cancelled."; returnMenu(); return;
...
get_status: // ===== STATUS =====
    gotoxy(1,9); clreol(); info("[!] Enter (available, in use, maintenance, not working)");
    gotoxy(1,8); clreol(); cout << "Status          : "; gets(ans1);
    for (i = 0; ans1[i]; i++) lowerA[i] = tolower(ans1[i]); lowerA[i] = '\0';
    int validStatus =
        strcmp(lowerA, "available") == 0 ||
        strcmp(lowerA, "in use") == 0 ||
    ...
    if (strlen(ans1) == 0 || ans1[0] == ' ' || ans1[0] == '\t') {
        ...
        gotoxy(19,8); error(" Invalid input."); info(" Press any key to continue."); getch(); goto get_status;
    } else strcpy(eq.status, lowerA);
    ...
    // ===== USAGE TRACKING ADDITION =====
    if (strcmp(eq.status, "in use") == 0) {
        struct date d; getdate(&d);
        struct time t; gettime(&t);
        sprintf(eq.lastUsedStart, "%02d/%02d/%04d %02d:%02d",
                d.da_day, d.da_mon, d.da_year, t.ti_hour, t.ti_min);
        eq.inUseFlag = 1;
    } else {
        eq.inUseFlag = 0;
        strcpy(eq.lastUsedStart, "-");
    }
...
//     char name[50], lowerName[50], input[100];
//     int i, found = -1;
...
//     // ==== CONDITION ====
//     gotoxy(1,7); cout << "New condition (very bad/bad/good/very good): ";
//     gets(input);
//     if (strlen(input) > 0 && !isCancel(input)) strcpy(eq.condition, input);

//     // ==== HOURS ====
//     gotoxy(1,8); cout << "New hours used: ";
//     gets(input);
//     if (strlen(input) > 0 && valid("float", input)) eq.usageHours = atof(input);

//     // ==== STATUS ====
//     gotoxy(1,9); cout << "New status (available/in use/maintenance/not working): ";
//     gets(input);
//     if (strlen(input) > 0 && !isCancel(input)) strcpy(eq.status, input);
...
get_name:
    ...
    for (i = 0; i < eqCounter; i++) {
        char temp[50];
        for (int j = 0; equipments[i].name[j]; j++) temp[j] = tolower(equipments[i].name[j]);
        temp[strlen(equipments[i].name)] = '\0';
        if (strcmpi(equipments[i].name, name) == 0 || strstr(temp, name)) { found = i; break; }
    }
    ...
    gotoxy(1,9); cout << "New status (available/in use/maintenance/not working): ";
    gets(input);
    if (strlen(input) > 0 && !isCancel(input)) {

        char oldStatus[30]; strcpy(oldStatus, eq.status);
        char lowerStatus[30];
        for (int k = 0; input[k]; k++) lowerStatus[k] = tolower(input[k]);
        lowerStatus[strlen(input)] = '\0';

        // ===== USAGE TRACKING =====
        if (strcmp(oldStatus, "in use") != 0 && strcmp(lowerStatus, "in use") == 0) {
        ...
        else if (strcmp(oldStatus, "in use") == 0 && strcmp(lowerStatus, "available") == 0 && eq.inUseFlag == 1) {
            // → Transition from IN USE to AVAILABLE
            struct date d2; getdate(&d2);
            struct time t2; gettime(&t2);

            int endMinutes = d2.da_day * 1440 + t2.ti_hour * 60 + t2.ti_min;
            int startDay, startMonth, startYear, startHour, startMin;
            sscanf(eq.lastUsedStart, "%d/%d/%d %d:%d", &startDay, &startMonth, &startYear, &startHour, &startMin);
            int startMinutes = startDay * 1440 + startHour * 60 + startMin;

            int duration = endMinutes - startMinutes;
            float durationHours = (float)duration / 60.0;
            if (durationHours < 0) durationHours = 0;

            eq.usageHours += durationHours;
            eq.inUseFlag = 0;
            strcpy(eq.lastUsedStart, "-");
        }

        strcpy(eq.status, lowerStatus);
    }
...
void showSummary() {
    ...
    gotoxy(1,11); cout << "Average Efficiency    : " << avgEff << "%";
    gotoxy(1,12); cout << "Average Availability  : " << avgAvail << "%";
    gotoxy(1,13); cout << "Operational Rate      : " << operationalPercent << "%";
